...
async function sendMailWithRetry(transporter, mailOptions, label, maxAttempts = 3) {
  let lastError;
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    try {
      ...
      lastError = err;
      console.error(`[Email] ${label} attempt ${attempt}/${maxAttempts} failed:`, err.message || err);
...
async function dispatchEmailNotification(name, email, number, inquiryType, message) {
  ...
  const [ownerResult, senderResult] = await Promise.allSettled([
    sendMailWithRetry(transporter, mailToOwner, 'Owner notification'),
    sendMailWithRetry(transporter, mailToSender, 'Sender confirmation'),
  ]);
...
function buildMonitorAlertHtml({ title, siteUrl, checkedAt, failures, recovered }) {
  const rows = (failures || [])
    .map(
      (f) =>
        `<tr><td style="padding:8px 0;color:#999;">${escapeHtml(f.name)}</td><td style="padding:8px 0;color:#fff;">${escapeHtml(f.error || 'Failed')}</td></tr>`
    )
    .join('');

  const accent = recovered ? '#22c55e' : '#ff5500';
  const headline = recovered ? '✅ Portfolio is back online' : '⚠️ Portfolio monitor alert';

  return `
<!DOCTYPE html>
<html lang="en"><head><meta charset="utf-8" /><meta name="viewport" content="width=device-width, initial-scale=1.0" /></head>
<body style="margin:0;padding:0;background:#050505;">
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#050505;padding:16px 0;">
    <tr><td align="center">
      <table role="presentation" width="100%" style="max-width:600px;background:#0a0a0a;border:1px solid ${accent};border-radius:12px;">
        <tr><td style="padding:24px;font-family:Arial,sans-serif;color:#fff;">
          <h2 style="margin:0 0 12px;color:${accent};font-size:22px;">${headline}</h2>
          <p style="margin:0 0 8px;color:#ddd;line-height:1.6;"><strong>${escapeHtml(title)}</strong></p>
          <p style="margin:0 0 16px;color:#aaa;font-size:14px;">Site: <a href="${escapeHtml(siteUrl)}" style="color:#ff5500;">${escapeHtml(siteUrl)}</a><br/>Checked: ${escapeHtml(checkedAt)}</p>
          ${rows ? `<table role="presentation" width="100%" style="border-collapse:collapse;margin-top:8px;">${rows}</table>` : ''}
          <p style="margin:24px 0 0;color:#666;font-size:12px;">Automated Portfolio Monitor</p>
        </td></tr>
      </table>
    </td></tr>
  </table>
...
async function sendMonitorAlert({ subject, siteUrl, checkedAt, failures, recovered = false }) {
  const { transporter, gmailUser, configured } = createTransporter();
  if (!configured || !transporter) {
    console.error('[Monitor] Cannot send alert — GMAIL_APP_PASS not configured.');
    return false;
  }
  ...
  await sendMailWithRetry(
    transporter,
    {
      from: `"Portfolio Monitor" <${gmailUser}>`,
      to: gmailUser,
      subject,
      priority: 'high',
      html: buildMonitorAlertHtml({
        title: subject,
        siteUrl,
        checkedAt,
        failures,
